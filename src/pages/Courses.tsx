...
import {
  BookOpen, CheckCircle2, Clock, Download, Sparkles, Filter, ChevronRight, Share2
} from 'lucide-react';
...
export const Courses: React.FC<CoursesProps> = ({ courses, loading = false, onOpenDemoModal }) => {
  ...
  const filteredCourses = courses.filter((c) => {
    ...
    const matchesDuration = selectedDuration === 'all'
      ...
      ? c.duration_weeks > 4 && c.duration_weeks <= 8
      : c.duration_weeks > 8;
  ...
  return (
    <div className="space-y-12 py-8 sm:py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <SEO
        title="Courses & Syllabus | Spoken English, IELTS, PTE, OET Coaching"
        description="Explore comprehensive English and international test preparation programs at EESA Academy. Small batches (max 8), daily 1-on-1 speaking, and computer practice labs."
      ...
      <section className="bg-gradient-to-b from-slate-900 via-blue-950 to-slate-900 text-white rounded-3xl p-8 sm:p-12 shadow-2xl border border-slate-800 relative overflow-hidden">
        <div className="max-w-3xl space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <LastUpdatedBadge dateString="August 2026" version="Curriculum v4.2" className="bg-slate-800 text-slate-200 border-slate-700" />
          </div>
          ...
          <div className="pt-2 flex flex-wrap gap-3">
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-black px-6 py-3 rounded-xl shadow-lg transition-all text-xs flex items-center gap-2 cursor-pointer"
            >
            ...
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-3 rounded-xl shadow-md transition-all text-xs flex items-center gap-2 cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
            ...
            <div className="md:col-span-4">
              <select
                ...
                className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-sm font-semibold text-slate-800 dark:text-slate-200 outline-none focus:ring-2 focus:ring-blue-600 cursor-pointer"
              >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {filteredCourses.map((course) => (
              <div
                key={course.id}
                className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-2xl hover:-translate-y-1 hover:border-blue-500/40 transition-all duration-300 flex flex-col justify-between group"
              >
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <CopyButton textToCopy={`EESA Academy - ${course.title}: ${course.short_description}`} iconOnly />
                    </div>
                  </div>
                ...
                <div className="pt-6 border-t border-slate-200 dark:border-slate-800 mt-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                  ...
                  <button
                    ...
                    className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-xl shadow-md transition-all text-xs flex items-center justify-center gap-2 cursor-pointer hover:shadow-lg"
                  >
                    <Sparkles className="w-4 h-4" />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-gradient-to-r from-slate-900 to-blue-950 rounded-3xl p-8 text-white border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
          ...
          <button
            onClick={() => onOpenDemoModal(undefined, 'prospectus_download')}
            className="bg-emerald-600 hover:bg-emerald-700 text-white font-black px-7 py-3.5 rounded-xl shadow-lg transition-all text-xs flex items-center gap-2 shrink-0 cursor-pointer"
          >
            <Download className="w-4 h-4" />
